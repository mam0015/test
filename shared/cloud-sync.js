(function(global){
  ...
  async function context(){await global.ACAuth?.ready;const profile=global.ACAuth?.profile();if(!global.ACAuth?.hasAccess?.()||!profile?.organisation_id)return null;return{profile,headers:{apikey:config.publishableKey,'Content-Type':'application/json',...(await global.ACAuth.headers())}}}
  ...
  async function pull(){
    ...
    try{const response=await fetch(endpoint(ctx.profile)+'&select=organisation_id,workspace,revision,updated_at',{headers:ctx.headers});if(!response.ok)throw new Error(`Cloud sync ${response.status}`);const row=(await response.json())[0],local=global.ACProjects.snapshot(),localTime=localStamp(local),remoteTime=String(row?.updated_at||'');lastRemote=remoteTime;
      ...
      else if(local.projects.length&&(!remoteProjects.length||localTime>remoteTime))await push(true);else status('online',`Last synced ${new Date().toLocaleTimeString('en-AU',{hour:'numeric',minute:'2-digit'})}`);
